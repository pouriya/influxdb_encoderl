...
RELEASE_DIR  = $(CURDIR)/_build/default/rel/influxdb_encoderl
VERSION     := $(shell erl -noshell -eval 'io:format("~s", [case file:consult("$(CURDIR)/src/influxdb_encoderl.app.src") of {ok, [{_, _, Terms}]} -> proplists:get_value(vsn, Terms, "0.0.0"); _ -> "0.0.0" end]),erlang:halt().')
RELEASE_NAME = influxdb_encoderl-$(VERSION)
NAME_UPPER  := $(shell echo influxdb_encoderl | awk '{print toupper($$1)}')
...
compile:
	...
	$(PRE) \
            ...
            $(REBAR) compile \
        $(POST)
	$(PRE) cp -r $(CURDIR)/_build/default/lib/influxdb_encoderl/ebin $(CURDIR)
...
shell:
	...
	$(PRE) \
        ...
        erl     -pa `ls -d _build/default/lib/*/ebin` \
                ...
                -eval "begin application:load('influxdb_encoderl'), catch code:load_file('influxdb_encoderl') end" \
                +B
...
docker:
	$(PRE) docker build -t influxdb_encoderl ./ $(POST)
